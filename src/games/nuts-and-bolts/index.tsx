...
import type { BoltType, NutType } from "./space-type";
  ...
  return (
    <Game status={[["Level", level]]} onRestart={doRestartAction}>
      <div className="flex h-full w-full flex-wrap items-center justify-evenly gap-x-4 rounded-sm bg-slate-700 p-8">
        {space.bolts.map(BoltNode)}
      </div>
    </Game>
  );
}

function BoltNode({ nuts, size, state }: BoltType, index: number) {
  return (
    <div
      key={index}
      ...
      className={cn(
        ...
        state != "complete" && "hover:cursor-pointer",
      )}
    >
      {/* bolt body */}
      ...
      {nuts.map((nut, index) => (
        <NutNode nut={nut} key={index} />
      ))}
    </div>
  );
}

function NutNode({ nut }: { nut: NutType }) {
  return (
    <div
      className={cn(
